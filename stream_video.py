...
OVERLAY_TEXT = "LIVE STREAM"  # Text overlay on the video

def restream(video_url, rtmp_url, overlay_image=None, overlay_text="LIVE STREAM"):
    """Re-streams a video or audio stream to an RTMP server with an optional overlay."""
    
    video_url_escaped = shlex.quote(video_url)
    overlay_text = overlay_text.replace(":", r"\:").replace("'", r"\'").replace('"', r'\"')
    ...
    if overlay_image:
        ...
        command += [
            "-vf", f"drawtext=text='{overlay_text}':fontcolor=white:fontsize=24:x=20:y=20"
        ]
...
if __name__ == "__main__":
    restream_video(VIDEO_URL, RTMP_URL, OVERLAY_IMAGE, OVERLAY_TEXT)